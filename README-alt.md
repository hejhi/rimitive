...
Adapters provide three minimal primitives:

```typescript
interface AdapterPrimitives {
  atom<T>(initial: T): Atom<T>;      // Reactive state
  computed<T>(fn: () => T): Computed<T>;  // Lazy computed values
  effect(fn: () => void): void;      // Side effects
}
```

Lattice transforms your rich specifications into operations using only these primitives.
