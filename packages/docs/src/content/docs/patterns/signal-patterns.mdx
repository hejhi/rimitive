...
<SearchTags tags={["signal", "toggle", "previous-value", "debounce", "async-action", "derived-state"]} />

## Try It

<SandboxDir dir="signal-patterns" editorHeight="420px" />
...
<Aside type="note">
**Exception:** Intentional global state (like a theme or auth state) can be module-level, but should be explicit and documented as such. See [Shared State](/patterns/shared-state) for the proper approach.
</Aside>
...
<Aside type="tip">
Use `signal.peek()` when you intentionally want to read without tracking—for example, in event handlers where you need the current value but don't want to create a dependency.
</Aside>
