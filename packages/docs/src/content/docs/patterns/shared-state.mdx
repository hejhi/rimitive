...
Lattice doesn't have a separate context primitive. It doesn't need one.

---

## The Pattern: Service Threading

In Lattice, the **service is the context**. Components receive a service object containing the primitives they need. You can extend that service with additional state at any point in the tree.
...
Lattice components are just functions returning specs. There's no render cycle or "currently rendering" context to hook into. Any context system would either:

1. Require explicit scope passing (which is what we already have)
2. Add hidden magic that fights Lattice's design philosophy
...
<Aside type="tip">
If you find yourself passing the same service through many layers, consider whether those intermediate components actually need to be separate. Lattice's composition model often leads to flatter component hierarchies than you might expect.
</Aside>
