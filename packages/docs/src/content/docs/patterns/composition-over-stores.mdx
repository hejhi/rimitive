...
description: Why Lattice doesn't have stores, and what to do instead.
---
...
Lattice doesn't have stores. This is intentional.
...
There's nothing saying you can't create your own primitive—it's just that lattice doesn't ship one by default.
...
```typescript
const checkout = ({ use, computed }: SignalsSvc) => {
  const useCart = use(cart);
  const useUserProfile = use(userProfile);
...
If you're convinced you need proxy-based stores, you can build them on top of Lattice's primitives. The signals are there—wrap them in proxies if you want.
...
But consider: if you're reaching for proxies, you might be fighting the framework. Lattice's composition model works best when you embrace it.
