...
Sometimes you need direct DOM access—for focus management, measurements, third-party library integration, or canvas drawing. Lattice handles this with the `.ref()` method on elements.
...
The callback receives a cleanup registration function:
...
const Chart = (svc: Service, props: { data: Readable<ChartData> }) => {
  ...
  return el('canvas').ref((canvas, cleanup) => {
...
For collections, store refs in an array or map:
