...
Node type (element vs fragment) is determined by `ref.status`<!-- -->: - STATUS\_ELEMENT (1): Element node with actual DOM element - STATUS\_FRAGMENT (2): Fragment node (logical container, no DOM element)
