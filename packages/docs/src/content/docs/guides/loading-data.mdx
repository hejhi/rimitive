...
`resource()` starts fetching immediately when created—it doesn't wait for hydration or know about server vs client. In SSR apps:

- Use `load()` for data that should be server-rendered
- Use `resource()` only for client-side dynamic data (filters, search, user interactions)
- If you need a resource that only runs on the client, guard its creation:

```typescript
const data = typeof window !== 'undefined'
  ? resource((s) => fetch('/api/data', { signal: s }).then(r => r.json()))
  : null;
```
</Aside>
