...
> IM-POR-TANT! Before we proceed, let's again differentiate rimitive from most other reactive frameworks, and understand that `el` itself **never re-renders**. You can pass it reactive children, you can pass it reactive props, and those will all update when they change, but the underlying tag/element is stable. The element is created a single time. There is no VDOM here, only fine-grained updates.
>
...
>
> This is **great for performance**, creates a **much simpler mental model**, and means that if you don't need reconciliation, don't use it, and **save some kbs, yo 📦**.
...
> **Important:** Differentiation alert, again! Components in rimitive are not magical reactive closures! They are just functions. Just another pattern. There's no re-rendering, just reactive primitives. You're simply returning an element tree.
