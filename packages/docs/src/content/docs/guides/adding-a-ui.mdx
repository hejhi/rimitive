...
There _are_ other primitives (like `match` and `map`) that can dynamically render `el`. But `el` itself is dumb. It doesn't reconcile or re-render. 

This is **great for performance**, creates a **much simpler mental model**, and means that if you don't need reconciliation, don't use it, and **save some kBs 📦**.
</Aside>
