...
3. A way to pass that data to the client to provide **back** into your app (`hydrationData`), which completes the cycle by providing it back into the respective `load()` primitives (and thus hydrating them) 🔄
...
SERVER
  │
  ├─ App runs, hits load() calls
  ├─ Fetches data, waits for all
  ├─ Renders HTML with fetched data
  ├─ Packs fetched data into a <script> tag
  │
  ▼
─────────── HTML + Data ───────────►
  │
...
CLIENT
  │
  ├─ Displays HTML (instant)
  ├─ Loads JS, reads packed data
  ├─ App runs, load() finds respective cached data
  └─ Hydrates without refetching or re-rendering ✓
  │
  🎉 UI interactive!
```

<Aside type="tip">
You can nest `load()` boundaries! The server resolves them iteratively—outer boundaries first, then any inner boundaries that appear after outer data loads.
</Aside>
...
1. **`renderToStringAsync`** — Waits for all `load()` fetchers to complete
2. **`insertFragmentMarkers`** — Adds markers around `load()` boundaries for hydration
3. **`service.loader.getData()`** — Returns a map of all fetched data (ID → data)
4. **`safeJsonStringify`** — Safely embeds JSON in HTML (prevents XSS from `</script>` in data)
...
- **[Streaming SSR](/guides/streaming-ssr)** — Send the shell immediately, stream data as it loads
