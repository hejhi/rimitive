...
description: Render Lattice apps on the server with data loading.
---

Server-side rendering in Lattice renders your components to HTML on the server, waits for async data to load, then sends the complete page to the browser. The client hydrates the existing DOM instead of recreating it.

Unlike many frameworks, Lattice effects are synchronous and run on the server too. There's no special "client-only" mode — your reactive code works the same way in both environments. This makes SSR predictable: signals update, effects run, DOM changes, all synchronously.
