...
Components use `load()` exactly the same as [basic async SSR](/guides/ssr-loading#using-load).
...
<Aside type="tip">
Both approaches use the same `load()` API in components—only the server setup differs. You can switch between them without changing component code.
</Aside>
