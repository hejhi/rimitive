...
The key insight: signals update the UI. The server streams data, the client pushes it to signals, and Lattice handles the rest.
...
const fetchNews = async (): Promise<News> => {
  ...
  return { headlines: ['Breaking: Lattice is fast', 'Streaming SSR works'] };
};
