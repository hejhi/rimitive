...
// Create a dropdown component that accepts a slice factory
export const createDropdown = (createSlice) => {
  ...
  // Return the complete dropdown behavior
  return { dropdown: keyboardDropdown };
};
```
...
<Tabs>
  <TabItem label="React">
    ...
    function SelectMenu({ items, onSelect }) {
      const createSlice = createStore({ items, isOpen: false, highlightedIndex: 0, selectedItem: null });
      const { dropdown } = createDropdown(createSlice);
      
      const isOpen = useSliceSelector(dropdown, s => s.isOpen());
      const highlightedIndex = useSliceSelector(dropdown, s => s.highlightedIndex());
      const { toggle, handleKeyDown, close } = dropdown.selector;
  ...
  <TabItem label="Vue">
    ...
    const createSlice = createStore({ items: props.items, isOpen: false, highlightedIndex: 0, selectedItem: null });
    const { dropdown } = createDropdown(createSlice);
    
    const isOpen = useSlice(dropdown, s => s.isOpen());
    const highlightedIndex = useSlice(dropdown, s => s.highlightedIndex());
    const { toggle, handleKeyDown, close } = dropdown.selector;
    </script>
...
describe('Dropdown behavior', () => {
  it('handles keyboard navigation', () => {
    ...
    const { highlightedIndex, handleKeyDown } = dropdown.selector;
    ...
    expect(dropdown.selector.isOpen()).toBe(false);
  });
});
```
