...
Your app is JavaScript, and JavaScript can run on the server. For view primitives like `el`, Rimitive lets you swap in a server-friendly adapter that generates markup without a real DOM.

The out-of-the-box server adapter is powered by [`linkedom`](https://github.com/WebReflection/linkedom), a lightweight DOM implementation. When `el("div")` runs on the server, it creates a linkedom element instead of a real DOM element. This lets linkedom render your app's HTML structure the same way the browser would.
...
Rimitive supports data fetching during SSR with a primitive called `load()`.
...
1. **Server renders your app** — using a server adapter that creates linkedom elements
