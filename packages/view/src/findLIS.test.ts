...
// Helper to create buffers for testing
function runLIS(arr: number[], n: number = arr.length): { len: number; lisBuf: number[] } {
  const lisBuf: number[] = [];
  const tailsBuf: number[] = new Array(n);
  const parentBuf: number[] = new Array(n);
  const len = findLIS(arr, n, lisBuf, tailsBuf, parentBuf);
  return { len, lisBuf };
}
...
describe('findLIS (Longest Increasing Subsequence)', () => {
  ...
  it('handles reverse sorted (descending) sequence', () => {
    const { len, lisBuf } = runLIS([5, 4, 3, 2, 1], 5);
    expect(len).toBe(1);
  ...
  it('handles classic example [10, 9, 2, 5, 3, 7, 101, 18]', () => {
    const { len, lisBuf } = runLIS([10, 9, 2, 5, 3, 7, 101, 18], 8);
    expect(len).toBe(4);
  ...
  it('handles sequence with duplicates', () => {
    const { len, lisBuf } = runLIS([1, 3, 2, 3, 4], 5);
    expect(len).toBe(4);
  ...
  it('handles partial array using n parameter', () => {
    ...
    const { len, lisBuf } = runLIS(arr, 3); // Only consider first 3 elements
    expect(len).toBe(3);
    expect(lisBuf.slice(0, len)).toEqual([0, 1, 2]);
  });

  it('handles complex reordering case', () => {
    const { len, lisBuf } = runLIS([3, 1, 4, 1, 5, 9, 2, 6], 8);
    expect(len).toBe(4);
  ...
  it('verifies indices are in ascending order', () => {
    const { len, lisBuf } = runLIS([5, 2, 8, 6, 3, 6, 9, 7], 8);
    const indices = lisBuf.slice(0, len);
