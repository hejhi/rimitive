...
describe('select', () => {
  ...
  it('should work with no dependencies', () => {
    ...
    const constantView = compute(
      () => () => ({ fixed: 100, message: 'No deps needed' })
    );
  ...
  it('should support multiple parameters', () => {
    const model = createModel<{ x: number; y: number }>(() => ({ x: 5, y: 10 }));
    ...
    const compute = select(model, { coord: coordSlice });

    const distanceView = compute(
      ({ coord }) => (x2: number, y2: number) => {
        const dx = coord.x() - x2;
        const dy = coord.y() - y2;
  ...
  it('should support rest parameters', () => {
    const model = createModel<{ items: number[] }>(() => ({ items: [1, 2, 3] }));
    
    const itemsSlice = createSlice(model, (m) => ({
      all: () => m().items
    }));

    const compute = select(model, { items: itemsSlice });
  ...
  it('should cache slice execution per adapter context', () => {
    ...
    const compute = select(model, { tracking: trackingSlice });
  ...
  it('should maintain referential equality for memoized results', () => {
    ...
    const compute = select(model, { data: dataSlice });

    const transformView = compute(
      ({ data }) => (prefix: string) => ({
        transformed: {
          id: data.get().id,
  ...
  it('should create independent compute functions from the same select', () => {
    const model = createModel<{ a: number; b: number }>(() => ({ a: 10, b: 20 }));
    ...
    const compute = select(model, { a: aSlice, b: bSlice });
