 ...
 */
export const LATTICE_STATE_MARKER = Symbol('LATTICE_STATE');
 ...
 */
export function markAsLatticeState<
  T extends StateInstance<any> & { [LATTICE_STATE_MARKER]?: boolean },
>(value: T): T & StateInstance<any> {
  value[LATTICE_STATE_MARKER] = true;
  return value;
}
...
if (import.meta.vitest) {
  ...
  describe('isLatticeState', () => {
    it('should correctly identify marked vs unmarked functions', () => {
      ...
      unmarkedFunction[LATTICE_STATE_MARKER] = true;
  ...
  describe('markAsLatticeState', () => {
    it('should mark a function as a Lattice state', () => {
      ...
      expect(regularFunction[LATTICE_STATE_MARKER]).toBeUndefined();

      // After marking
      // @ts-expect-error - this is not an actual state instance
