...
// Enhanced selectors type
type EnhancedSelectors = {
  doubled: number;
  isEven: boolean;
};
...
describe('Selectors Composition', () => {
  ...
  it('should create selectors that access model properties and compute values', () => {
    ...
    const counterSelectors = createSelectors<CounterSelectors & { 
  ...
  it('should support creating enhanced selectors with additional derived values', () => {
    ...
    const enhancedSelectors = createSelectors<CounterSelectors & {
      doubled: number;
      isEven: boolean;
    }, CounterModel>(
      { model: mockModel },
  ...
  it('should support fluent compose API for selectors', () => {
    // Create base selectors
    const baseSelectors = createSelectors<CounterSelectors, CounterModel>(
      { model: mockModel },
      ({ model }) => ({
        count: model().count,
        isPositive: model().count > 0,
    ...
    // Use the fluent compose API with .with() method following lines 154-160
    const composed = compose(baseSelectors).with<EnhancedSelectors, CounterModel>(
      ({ model }) => ({
        doubled: model().count * 2,
        isEven: model().count % 2 === 0,
      })
    ...
    expect(isSelectorsFactory(composed)).toBe(false);
    
    // But wrapping it with createSelectors should brand it
    const wrappedSelectors = createSelectors<CounterSelectors & EnhancedSelectors, CounterModel>(
      { model: mockModel },
      (_) => {
        // Need to adapt our composed function to work with the expected API
        // In this case we need a proper get function that returns the combined selectors
        const mockGet = () => ({
          count: mockModel.count,
          isPositive: mockModel.count > 0,
          doubled: mockModel.count * 2,
          isEven: mockModel.count % 2 === 0
        });
        
        return composed({ get: mockGet });
      }
    );
