...
import { compose, createModel, createSlice } from '.';
...
describe('compose', () => {
  ...
  it('should handle multiple dependencies', () => {
    ...
    const colorSlice = createSlice(model, (m) => ({
      primary: () => m().colors.primary,
      secondary: () => m().colors.secondary,
    }));
    const sizeSlice = createSlice(model, (m) => ({
      width: () => m().sizes.width,
  ...
  it('should work with nested slice compositions', () => {
    ...
    const cSlice = createSlice(model, (m) => ({
      value: m().c,
    }));
    ...
    const abSlice = createSlice(
      ...
      compose({ a: aSlice, b: bSlice, op: opSlice }, (_, { a, b, op }) => ({
        result: op.operation === 'add' ? a.value + b.value : a.value * b.value,
      }))
    );
    ...
    const multiplyResult = finalSlice(() => ({
      a: 5,
      b: 3,
      c: 2,
      op: 'multiply',
