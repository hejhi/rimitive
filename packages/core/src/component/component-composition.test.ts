...
describe('Component Composition', () => {
  ...
  it('should properly track dependencies in computed values', () => {
    ...
    expect(component.filtered()[0]?.text).toBe('Buy milk');

    component.setFilter('done');
    expect(component.filtered()).toHaveLength(1);
    expect(component.filtered()[0]?.text).toBe('Read book');
  });
  ...
  it('should support nested components', () => {
    ...
    const UserComponent = ({
      ...
      user: store.user,
      updateName: (name: string) => set(store.user, { ...store.user(), name }),
    });
    ...
    const SettingsComponent = ({
      ...
      toggleTheme: () =>
        set(store.settings, {
          ...store.settings(),
          theme: store.settings().theme === 'light' ? 'dark' : 'light',
        }),
    });
    ...
    expect(app.user.user().name).toBe('John');
    expect(app.settings.settings().theme).toBe('light');

    app.user.updateName('Jane');
    expect(app.user.user().name).toBe('Jane');

    app.settings.toggleTheme();
    expect(app.settings.settings().theme).toBe('dark');
  });
});
