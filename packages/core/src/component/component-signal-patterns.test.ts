...
/**
 * Best practices for signal usage in components
 */
...
describe('Component Signal Patterns - Best Practices', () => {
  ...
  it('should use signal.set for nested object/array updates', () => {
    ...
    const TodoList: Component<TodoState, {
      ...
      return {
        get todos() { return store.state.todos.value; },
  ...
  it('should support fine-grained subscriptions to individual signals', () => {
    ...
    const Form: Component<FormState, {
      username: string;
      email: string;
      ...
      return {
        get username() { return store.state.username.value; },
        get email() { return store.state.email.value; },
  ...
  it('should demonstrate performance patterns with effects', () => {
    ...
    const Dashboard: Component<DashboardState, {
      activeUsers: number;
      totalRevenue: number;
      ...
      return {
        get activeUsers() { return store.state.activeUsers.value; },
        get totalRevenue() { return store.state.totalRevenue.value; },
  ...
  it('should use computed values with direct signal access', () => {
    ...
    const ShoppingCart: Component<ShoppingCartState, {
      ...
      return {
        get items() { return store.state.items.value; },
        get subtotal() { return subtotal.value; },
        get tax() { return tax.value; },
        get discount() { return discount.value; },
  ...
  it('should demonstrate select pattern for fine-grained computed tracking', () => {
    ...
    const App: Component<AppState, {
      ...
      return {
        ...
        get hasNotifications() { return hasNotifications.value; },
    ...
    const unsubName = store.state.user.select(u => u.name).subscribe(() => {
      nameChanges++;
    });
    
    const unsubRole = store.state.user.select(u => u.role).subscribe(() => {
      roleChanges++;
    });
