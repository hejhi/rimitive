...
// This enables better control of the transpiled output size.
function SignalImpl(this: Signal, value?: unknown, options?: SignalOptions) {
  this._value = value;
  this._version = 0;
...
// Cast to SignalConstructor
const Signal = SignalImpl as unknown as SignalConstructor;
...
Signal.prototype._subscribe = function (this: Signal, node: Node): void {
...
Signal.prototype._unsubscribe = function (this: Signal, node: Node): void {
...
Signal.prototype.valueOf = function <T>(this: Signal<T>): T {
  return this.value;
};

Signal.prototype.toString = function (this: Signal): string {
  return String(this.value);
};

Signal.prototype.toJSON = function <T>(this: Signal<T>): T {
  return this.value;
};
...
const Computed = ComputedImpl as unknown as ComputedConstructor;
...
Computed.prototype._refresh = function (this: Computed): boolean {
  ...
  try {
    ...
    if (flags & HAS_ERROR || this._value !== value || this._version === 0) {
      this._value = value;
...
Computed.prototype._subscribe = function (this: Computed, node: Node): void {
  if (this._targets === undefined) {
...
Computed.prototype._unsubscribe = function (this: Computed, node: Node): void {
...
interface EffectConstructor {
  new (fn: EffectFn): Effect;
  prototype: Effect;
}

function EffectImpl(this: Effect, fn: EffectFn) {
  this._fn = fn;
...
const Effect = EffectImpl as unknown as EffectConstructor;

Effect.prototype._callback = function (this: Effect): void {
...
Effect.prototype._start = function (this: Effect): () => void {
  if (this._flags & RUNNING) {
    throw new Error('Cycle detected');
  }
  this._flags |= RUNNING;
  ...
  // eslint-disable-next-line @typescript-eslint/no-this-alias
  evalContext = this;
  return () => endEffect.call(this, prevContext);
};

Effect.prototype._notify = function (this: Effect): void {
  if (!(this._flags & NOTIFIED)) {
    this._flags |= NOTIFIED;
...
Effect.prototype._dispose = function (this: Effect): void {
  this._flags |= DISPOSED;
...
Effect.prototype.dispose = function (this: Effect): void {
  this._dispose();
};
/**
 ...
 */
export function effect(fn: EffectFn): { (): void; [Symbol.dispose](): void } {
  ...
  const disposeFn = dispose as { (): void; [Symbol.dispose]: () => void };
  disposeFn[Symbol.dispose] = dispose;
  return disposeFn;
}

// Export the Signal constructor with its type
export { Signal };
