...
function isMarker(node: Node | null): boolean {
  ...
  return t === 'fragment-start' || t === 'fragment-end' ||
         t.startsWith('async:') || t.startsWith('/async:');
}
...
function getNthChild(parent: Node, index: number): Node {
  ...
  throw new HydrationMismatch(`Child at index ${index} not found (found ${count} children)`);
}
...
function findFragmentIndex(parent: Element, nextSibling: Element | null): number | null {
  // Walk backwards from nextSibling to find fragment-start
  let node: Node | null = nextSibling
  ...
  return {
    createNode: (type, props) => {
      ...
      if (type === 'text') {
        ...
        if (node.nodeType !== 3) {
          throw new HydrationMismatch(`Expected text node at ${path.join('/')}, got ${node.nodeName}`);
        }
      ...
      // Element nodes
      const node = getNodeAtPath(containerEl, path);
      if (node.nodeType !== 1 || (node as Element).tagName.toLowerCase() !== type.toLowerCase()) {
        throw new HydrationMismatch(`Expected <${type}> at ${path.join('/')}, got <${(node as Element).tagName}>`);
      }
    ...
    appendChild: (parent, child) => {
      ...
      if (child && 'nodeType' in child && (child as Node).nodeType === 1 &&
    ...
    beforeAttach: (ref: NodeRef<Node>, parentElement, nextSiblingElement) => {
      ...
      const index = findFragmentIndex(parentElement as HTMLElement, nextSiblingElement as HTMLElement);
...
export function createHydrationAdapter(
  ...
  return {
    ...
    insertBefore: (parent, newNode, refNode) => a.insertBefore(parent, newNode, refNode),
    ...
    switchToFallback: () => { a = fallbackAdapter; },
  };
}
