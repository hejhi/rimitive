...
async function measureBundle(name: string, entry: string): Promise<BundleResult> {
...
describe('Bundle Size Analysis', () => {
  ...
  bench('measure @lattice/core', async () => {
    const entry = join(__dirname, 'bundle-entries/core-only.ts');
    const code = `export * from '@lattice/core';`;
    mkdirSync(join(__dirname, 'bundle-entries'), { recursive: true });
  ...
  bench('measure @lattice/adapter-redux', async () => {
    const entry = join(__dirname, 'bundle-entries/adapter-redux.ts');
  ...
  bench('measure @lattice/adapter-zustand', async () => {
    const entry = join(__dirname, 'bundle-entries/adapter-zustand.ts');
  ...
  bench('measure @lattice/adapter-store-react', async () => {
    const entry = join(__dirname, 'bundle-entries/adapter-store-react.ts');
  ...
  bench('measure @lattice/runtime', async () => {
    const entry = join(__dirname, 'bundle-entries/runtime.ts');
  ...
  bench('measure complete Redux setup', async () => {
    const entry = join(__dirname, 'bundle-entries/complete-redux.ts');
    const code = `
  ...
  bench('measure complete Zustand setup', async () => {
    const entry = join(__dirname, 'bundle-entries/complete-zustand.ts');
    const code = `
      export * from '@lattice/core';
  ...
  bench('measure complete store-react setup', async () => {
    const entry = join(__dirname, 'bundle-entries/complete-store-react.ts');
    const code = `
      export * from '@lattice/core';
...
function generateReport(results: BundleResult[]): string {
  ...
  const individualPackages = results.filter(r => 
    r.name.startsWith('@lattice/') && !r.name.includes('Complete')
  );
...
describe('Bundle Size Comparison with State Libraries', () => {
  bench('measure raw Redux Toolkit', async () => {
    const entry = join(__dirname, 'bundle-entries/raw-redux.ts');
    const code = `
    ...
    console.log(`Redux Toolkit: ${(result.gzipSize / 1024).toFixed(2)} KB gzipped`);
  });

  bench('measure raw Zustand', async () => {
    const entry = join(__dirname, 'bundle-entries/raw-zustand.ts');
    const code = `
