...
- If not, it clears any DERIVED_DIRTY flag and returns.
- If it has, it marks itself as DERIVED_DIRTY and propagates CONSUMER_PENDING to all children

When a Consumer is executed:
- It looks at its direct Producers
- If any Producer is marked DERIVED_DIRTY, it recomputes
- That recomputation causes re-execution of any Producers in it, propagating "up"

QUESTIONS:
- For signals, do we need to mark all children as CONSUMER_PENDING?
- Right now, computeds don't mark children at all. Should they?
- Do we need CONSUMER_PENDING? Effects don't need it (they get scheduled). Computeds look for DERIVED_DIRTY on direct children and naturally propagate upwards. is DERIVED_DIRTY enough?