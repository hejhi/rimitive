/**
 * Component API Benchmarks
 *
 * Tests comparing Lattice's component API against Preact's raw signals
 * Note: This compares different abstraction levels (component vs raw signals)
 */
...
describe('Computed Chain (a → b → c)', () => {
  ...
  bench('Lattice - computed chain', () => {
    ...
    const Chain = ({
      ...
      set,
      computed,
    }: ComponentContext<{ a: number }>) => {
      const b = computed(() => store.a.value * 2);
...
describe('Diamond Pattern', () => {
  ...
  bench('Lattice - diamond', () => {
    ...
    const Diamond = ({
      ...
      const b = computed(() => store.a.value * 2);
      const c = computed(() => store.a.value * 3);
...
describe('Deep Computed Tree', () => {
  ...
  bench('Lattice - deep tree', () => {
    ...
    const DeepTree = ({
      ...
      set,
      computed,
    }: ComponentContext<{ root: number }>) => {
      // Level 1
      const a1 = computed(() => store.root.value * 2);
      const a2 = computed(() => store.root.value * 3);
