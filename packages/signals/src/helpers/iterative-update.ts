...
export interface IterativeUpdateHelpers {
  iterativeUpdate: (node: UpdatableNode, ctx: SignalContext) => void;
}

export function createIterativeUpdateHelpers(): IterativeUpdateHelpers {
  ...
  const returnFrame = (frame: UpdateFrame | undefined): void => {
    if (!frame) return;
  ...
  const isVisiting = (node: UpdatableNode): boolean => {
  ...
  const addVisiting = (node: UpdatableNode): void => {
    visiting.push(node);
  };

  // Helper to remove from visiting
  const removeVisiting = (node: UpdatableNode): void => {
    const len = visiting.length;
    for (let i = 0; i < len; i++) {
      if (visiting[i] === node) {
