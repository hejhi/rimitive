        ...
        get: (params: { id: NodeID }) => {
          ...
          return {
            ...
            onClick: (e: MouseEvent) => {
              ...
              selectionAPI.getState().selectNode(id, e.shiftKey);
            },
          };
        },
      }))
...
/* 
  ...
  // Set test data
  treeWithSelection.api.getState().setNodes(createTestTreeData());
  
  // Select a node
  treeWithSelection.api.getState().selectNode("node1");
  
  // Check if a node is selected
  const isSelected = treeWithSelection.api.getState().isSelected("node1");
