...
We'll use an incremental approach, but will NOT support backwards compatibility or leave ANY legacy code behind, as it is confusing and makes typing complex.
...
- [ ] Update `SliceFactory` type to include required `api` parameter
...
- [ ] Ensure existing tests are updated to match our new api
